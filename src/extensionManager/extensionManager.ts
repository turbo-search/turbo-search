...
export class extensionManager implements ExtensionManager {
    ...
    constructor(extensions: Extension[], error: TurboSearchCoreOptions["error"], coreVersion: string, turboSearchKit: TurboSearchKit) {
        this._error = error;
        ...
        this._extensions.map((extension) => {
            if (typeof extension.available !== "function") {
                ...
                if (extensionAvailable.success) {
                    ...
                    if (typeof this._error == "object" && "boolean" && this._error.strictAvailable) {
    ...
    checkDependence() {
        this._extensions.forEach((extension) => {
            ...
            if (extension.manifesto.coreDependence) {
                if (extension.manifesto.coreDependence !== "" && !compareDependenceVersion(this._coreVersion, extension.manifesto.coreDependence)) {
                    catchError("dependence", [extension.manifesto.name + " coreDependence is not match", "Requested Version : " + extension.manifesto.coreDependence, "Current version : " + this._coreVersion])
                }
            }
            ...
            if (extension.manifesto.dependence && typeof extension.manifesto.dependence !== "undefined" && Object.keys(extension.manifesto.dependence).length > 0) {
                Object.keys(extension.manifesto.dependence).forEach((dependenceExtensionName) => {
                    // 依存している拡張機能の情報
                    const dependenceExtension = this._extensions.find((extension) => extension.manifesto.name === dependenceExtensionName);
