...
export class JobManager {
    ...
    async subscribeJob(
        ...
        const result = addSubscribeJobDataSchema.safeParse({ id: id, jobId: jobId, callback: callback });
        if (!result.success) {
            catchError("jobSubscribeValidation", ["job subscribe validation error", "subscribe id : " + id, "error : " + result.error.message])
        } else {
            //同じidが存在する場合も関係なく追加
    ...
    async subscribeJobs(
        ...
        const result = addSubscribeJobsDataSchema.safeParse({ id: id, callback: callback });
        if (!result.success) {
            catchError("jobsSubscribeValidation", ["jobs subscribe validation error", "subscribe id : " + id, "error : " + result.error.message])
        } else {
            //同じidが存在する場合も関係なく追加
    ...
    async checkSubscribe(job: Job, status: JobStatus) {
        const subscribeJobList = await this.memoryStoreManager.find("jobSubscribes", "jobId", job.id) as { id: string, jobId: string, callback: SubscribeJobCallback }[];
        const subscribeJobsList = await this.memoryStoreManager.findAll("jobsSubscribes") as { id: string, callback: SubscribeJobCallback }[];
